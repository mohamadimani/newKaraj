...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card mb-4">
        <h5 class="card-header heading-color">{{ __('clerks.edit') }}</h5>
        @if (Session::has('error'))
        <div class="alert alert-danger" role="alert">
            {{ Session::get('error') }}
        </div>
        @endif
        <form id="general-form-validation" class="card-body" action="{{ route('clerks.update', $clerk) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row g-3">
                @if ($errors->any())
                ...
                <div class="col-md-4 col-sm-6 mb-3">
                    ...
                    <input
                        name="first_name"
                        value="{{ old('first_name', $clerk->user->first_name) }}"
                        type="text"
                        id="first-name"
                ...
                <div class="col-md-4 col-sm-6 mb-3">
                    ...
                    <input
                        name="last_name"
                        value="{{ old('last_name', $clerk->user->last_name) }}"
                        type="text"
                        id="last-name"
                        class="form-control text-start"
                ...
                <div class="col-md-4 col-sm-6 mb-3">
                    ...
                    <input
                        name="mobile"
                        value="{{ old('mobile', $clerk->user->mobile) }}"
                        type="text"
                        id="mobile"
                        class="form-control"
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input
                        ...
                        value="{{ old('birth_date') ?? $clerk->user->birth_date }}">
                </div>
                <div class="col-md-4 col-sm-6 mb-3">
                    <label class="form-label" for="gender">{{ __('users.gender') }}</label>
                    <select name="gender" id="gender" class="form-select">
                        <option value="">---</option>
                        <option value="male" {{ old('gender', $clerk->user->gender) == 'male' ? 'selected' : '' }}>{{ __('users.gender_male') }}</option>
                        <option value="female" {{ old('gender', $clerk->user->gender) == 'female' ? 'selected' : '' }}>{{ __('users.gender_female') }}</option>
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <input
                        ...
                        class="form-control dob-picker"
                        placeholder="{{ __('users.form.date_placeholder') }}"
                        value="{{ old('start_date') ?? ($clerk->start_date ? Verta::instance($clerk->start_date)->format('Y-m-d') : '') }}">
                </div>
                <div class="col-md-4 col-sm-6 mb-3">
                    ...
                    <select name="province_id" id="province" class="form-select">
                        ...
                        <option value="{{ $province->id }}" {{ old('province_id', $clerk->user->province_id) == $province->id ? 'selected' : '' }}>{{ $province->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select name="branch_id" id="branch" class="select2 form-select" placeholder="{{ __('branches.select_branch') }}">
                        ...
                        <option value="{{ $branch->id }}" {{ old('branch_id', $clerk->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 col-sm-6 mb-3">
                    ...
                    <label class="switch">
                        <input name="is_active" type="checkbox" class="switch-input" {{ old('is_active', $clerk->is_active) ? 'checked' : '' }}>
