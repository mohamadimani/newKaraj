...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card mb-4">
        <h5 class="card-header heading-color">{{ __('discounts.edit') }}</h5>
        ...
        <form id="general-form-validation" class="card-body" action="{{ route('discounts.update', $discount) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row g-3">
                @if ($errors->any())
                ...
                <div class="col-md-3">
                    ...
                    <input value="{{ old('title', $discount->title) }}" type="text" name="title" id="title" class="form-control" placeholder="{{ __('discounts.title_placeholder') }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label" for="code">{{ __('discounts.code') }}</label>
                    <input value="{{ old('code', $discount->code) }}" type="text" name="code" id="code" class="form-control" placeholder="{{ __('discounts.code_placeholder') }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label" for="amount">{{ __('discounts.amount') }}</label>
                    <input value="{{ old('amount', $discount->amount) }}" type="text" name="amount" id="amount" class="form-control" placeholder="{{ __('discounts.amount_placeholder') }}">
                </div>
                <div class="col-md-3">
                    ...
                    <select name="amount_type" id="amount_type" class="form-select">
                        ...
                        @foreach (AmountTypeEnum::cases() as $amountType)
                            <option value="{{ $amountType->value }}" {{ old('amount_type', $discount?->amount_type?->value) == $amountType->value ? 'selected' : '' }}>
                                {{ $amountType->name() }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <input value="{{ old('minimum_order_amount', number_format($discount->minimum_order_amount)) }}" type="text" name="minimum_order_amount" id="minimum_order_amount"
                        class="form-control" placeholder="{{ __('discounts.minimum_order_amount_placeholder') }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label" for="available_from">{{ __('discounts.available_from') }}</label>
                    <input data-jdp value="{{ old('available_from', georgianToJalali($discount->available_from, true)) }}" type="text" name="available_from" id="available_from" class="form-control"
                ...
                <div class="col-md-3">
                    ...
                    <input data-jdp value="{{ old('available_until', georgianToJalali($discount->available_until, true)) }}" type="text" name="available_until" id="available_until"
                        class="form-control " placeholder="{{ __('discounts.available_until_placeholder') }}">
                </div>
                <div class="col-md-3 col-sm-6">
                    ...
                    <select name="is_online" id="is_online" class="form-select">
                        ...
                        <option value="0" {{ $discount->is_online ? '' : 'selected' }}>حضوری</option>
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label" for="usage_limit">{{ __('discounts.usage_limit') }}</label>
                    <input value="{{ old('usage_limit', $discount->usage_limit) }}" type="number" name="usage_limit" id="usage_limit" class="form-control"
                        placeholder="{{ __('discounts.usage_limit_placeholder') }}">
                </div>
                <div class="col-md-3">
                    ...
                    <select name="discount_type" id="discount_type" class="form-select">
                        ...
                        @foreach (DiscountTypeEnum::cases() as $discountType)
                            <option class="" value="{{ $discountType->value }}" {{ old('discount_type', $discount?->discount_type?->value) == $discountType->value ? 'selected' : '' }}>
                                {{ $discountType->name() }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3 col-sm-6 mb-1 d-none" id="profession-id-container">
                    ...
                    <select name="profession_id" id="profession-id" class="select2 form-select" data-allow-clear="true" data-placeholder="{{ __('discounts.select_profession') }}">
                        ...
                        @foreach ($professions as $profession)
                            <option value="{{ $profession->id }}" {{ old('profession_id', $discount->profession_id) == $profession->id ? 'selected' : '' }}>
                                {{ $profession->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3 col-sm-6 mb-1 d-none" id="user-id-container">
                    ...
                    <select name="user_id" id="user-id" class="select2 form-select" data-allow-clear="true" data-placeholder="{{ __('discounts.select_user') }}">
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $discount->user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->fullName }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3 col-sm-6 mb-1 d-none" id="course-id-container">
                    ...
                    <select name="course_id" id="course-id" class="select2 form-select" data-allow-clear="true" data-placeholder="{{ __('discounts.select_course') }}">
                        ...
                        @foreach ($courses as $course)
                            <option value="{{ $course->id }}" {{ old('course_id', $discount->course_id) == $course->id ? 'selected' : '' }}>
                                {{ $course->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3 col-sm-6 mb-1 d-none" id="course-online-id-container">
                    ...
                    <select name="online_courses" id="online_courses" class="select2 form-select" data-allow-clear="true" data-placeholder="{{ __('discounts.select_course') }}">
                        ...
                        @foreach ($onlineCourses as $onlineCourse)
                            <option value="{{ $onlineCourse->id }}" {{ old('online_courses', $discount->online_courses) == $onlineCourse->id ? 'selected' : '' }}>
                                {{ $onlineCourse->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-12 mb-3 mt-3">
                ...
                @if (mb_strlen($discount->banner) > 0)
                    <img src="{{ asset('public/images/discounts/banners/' . $discount->banner) }}" class="mt-2 border rounded  " style="width: 50%px; height: 80px;">
                @endif
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        setDiscountType($('#discount_type').val());
        ...
        function setDiscountType(discountType) {
            ...
            if (discountType == '{{ DiscountTypeEnum::PROFESSION->value }}') {
                $('#profession-id-container').removeClass('d-none');
            } else if (discountType == '{{ DiscountTypeEnum::USER->value }}') {
                $('#user-id-container').removeClass('d-none');
            } else if (discountType == '{{ DiscountTypeEnum::COURSE->value }}') {
                $('#course-id-container').removeClass('d-none');
            } else if (discountType == '{{ DiscountTypeEnum::COURSE_ONLINE->value }}') {
