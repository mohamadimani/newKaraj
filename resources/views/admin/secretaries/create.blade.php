...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        <div class="card mb-4">
            <h5 class="card-header heading-color">{{ __('secretaries.create') }}</h5>
            @if (Session::has('error'))
                <div class="alert alert-danger" role="alert">
                    {{ Session::get('error') }}
                </div>
            @endif
            <form id="general-form-validation" class="card-body" action="{{ route('secretaries.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    @if ($errors->any())
                        <div>
                    ...
                    <div class="col-md-12 mb-3">
                        <label class="form-label" for="phones">شعبه ها</label>
                        <select name="phones[]" id="phones" class="select2 form-select" multiple data-allow-clear="true" data-placeholder="{{ __('secretaries.select_phones') }}">
                            @foreach ($phones as $phone)
                                <option value="{{ $phone->id }}" {{ in_array($phone->id, old('phones', [])) ? 'selected' : '' }}>{{ $phone->title }} - {{ $phone->number }}
                                    ({{ $phone->branch->name }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    {{-- <div class="col-md-3 col-sm-6 mb-3">
                        <label class="form-label" for="internal_phone">شماره داخلی</label>
                        <input name="internal_phone" value="{{ old('internal_phone') }}" type="text" id="internal_phone" class="form-control text-start">
                    </div> --}}
                </div>
