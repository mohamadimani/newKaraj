...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card mb-4">
        <h5 class="card-header heading-color">{{ __('courses.edit') }}</h5>
        @if ($errors->any())
        <div>
            <ul>
        ...
        <form id="general-form-validation" class="card-body" action="{{ route('courses.update', $course->id) }}" method="POST">
            @csrf
            @method('PUT')
            <livewire:Admin.Courses.Update :course="$course" />
            ...
            <div class="row">
                <div class="col-md-3 mb-3">
                    ...
                    <input name="start_date" type="text" id="start-date" class="form-control dob-picker" placeholder="{{ __('users.form.date_placeholder') }}"
                        value="{{ old('start_date', georgianToJalali($course->start_date)) }}">
                </div>
                <div class="col-md-3 mb-3">
                    <label class="form-label" for="end-date">{{ __('courses.end_date') }}</label>
                    <input name="end_date" type="text" id="end-date" class="form-control dob-picker" placeholder="{{ __('users.form.date_placeholder') }}"
                        value="{{ old('end_date', georgianToJalali($course->end_date)) }}">
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <select name="start_time" id="start-time" class="form-control">
                        ...
                        @for ($i = 7; $i < 23; $i++)
                            @for ($j=0; $j < 60; $j +=15)
                            <option {{ old('start_time') == sprintf('%02d:%02d', $i, $j) || $course->start_time == sprintf('%02d:%02d:00', $i, $j) ? 'selected':'' }} value="{{ sprintf('%02d:%02d', $i, $j) }}">{{ sprintf('%02d:%02d', $i, $j) }}</option>
                            @endfor
                            @endfor
                    </select>
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <select name="end_time" id="end_time" class="form-control">
                        ...
                        @for ($i = 7; $i < 23; $i++)
                            @for ($j=0; $j < 60; $j +=15)
                            <option {{ old('end_time') == sprintf('%02d:%02d', $i, $j) || $course->end_time == sprintf('%02d:%02d:00', $i, $j) ? 'selected':'' }} value="{{ sprintf('%02d:%02d', $i, $j) }}">{{ sprintf('%02d:%02d', $i, $j) }}</option>
                            @endfor
                            @endfor
                    </select>
                </div>
                ...
                <div class="col-md-3 col-sm-6 mb-3">
                    ...
                    <select name="week_days[]" id="week_days" class="select2 form-select" multiple data-allow-clear="true" data-placeholder="{{ __('courses.week_days_placeholder') }}">
                        ...
                        <option value="{{ $weekDay['id'] }}" {{ in_array($weekDay['id'], $course->week_days ?? old('week_days')) ? 'selected' : '' }}>{{ $weekDay['name'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
