...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card mb-4">
        <h5 class="card-header heading-color">ایجاد آزمون</h5>
        @include('admin.layouts.alerts')
        <form id="general-form-validation" class="card-body" action="{{ route('exams.store') }}" method="POST">
            @csrf
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="profession_id[]" id="profession_id" class="select2 form-select" multiple placeholder="{{ __('courses.select') }}">
                        ...
                        <option value="{{ $profession->id }}" {{ old('profession_id') && in_array($profession->id,old('profession_id'))  ? 'selected' : '' }}>{{ $profession->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <select name="branch_id" id="branch_id" class="select2 form-select" placeholder="{{ __('courses.select') }}">
                        ...
                        <option value="{{ $branch->id }}" {{ old('branch_id',6) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6 col-sm-6 mb-3">
                    ...
                    <input name="description" value="{{ old('description') }}" type="text" id="description" class="form-control text-start" placeholder="توضیحات">
                </div>
                <div class="col-md-2 col-sm-6 mb-3">
                    <label class="form-label" for="duration_min">مدت آزمون (دقیقه)</label>
                    <input name="duration_min" value="{{ old('duration_min',30) }}" type="number" id="duration_min" class="form-control text-start"
                        placeholder="{{ __('courses.duration_hours_placeholder') }}">
                </div>
                <div class="col-md-2 col-sm-6 mb-3">
                    <label class="form-label" for="passing_score">نمره قبولی</label>
                    <input name="passing_score" value="{{ old('passing_score',50) }}" type="number" id="passing_score" class="form-control text-start"
                        placeholder="{{ __('courses.duration_hours_placeholder') }}">
                </div>
                <div class="col-md-2 col-sm-6 mb-3">
                    <label class="form-label" for="question_count">تعداد سوال</label>
                    <input name="question_count" value="{{ old('question_count',20) }}" type="number" id="question_count" class="form-control text-start"
