...
function numberToPersianText(number) {
  ...
  const ranks = ['', 'هزار', 'میلیون', 'میلیارد', 'تریلیون'];

  if (number < 0) {
    return 'منفی ' + numberToPersianText(Math.abs(number));
  }
  ...
  const padding = (3 - (numberStr.length % 3)) % 3;
  numberStr = '0'.repeat(padding) + numberStr;

  const groups = numberStr.match(/.{3}/g);

  function convertThreeDigits(num) {
    const sadgan_val = Math.floor(num / 100);
    const dahgan_val = Math.floor((num % 100) / 10);
    const yekan_val = num % 10;

    let result = [];

    if (sadgan_val > 0) {
      result.push(sadgan[sadgan_val]);
    }

    if (dahgan_val === 1) {
      result.push(dah[yekan_val]);
    } else {
      if (dahgan_val > 1) {
        result.push(dahgan[dahgan_val]);
      }
      if (yekan_val > 0) {
        result.push(yekan[yekan_val]);
      }
    }
  ...
  const parts = groups.map((group, index) => {
    ...
    const rank = ranks[groups.length - 1 - index];
    const converted = convertThreeDigits(number);
    return converted + (rank ? ' ' + rank : '');
  }).filter(part => part !== '');
